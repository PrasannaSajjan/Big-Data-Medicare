medicare_records = LOAD '/user/psajjan/Medicare-Physician-and-Other-Supplier-PUF-CY2012.txt' USING PigStorage('\t')
...
physician_records = LOAD '/user/psajjan/OPPR_ALL_DTL_GNRL_12192014.csv' USING org.apache.pig.piggybank.storage.CSVExcelStorage()
	...
	charity_indicator, third_party_equals_covered_recipient_indicator, contextual_information, delay_in_publication_of_general_payment_indicator);
						...
						-- Medicare records 
filtered_medicare = FOREACH(FILTER medicare_records BY hcpcs_code == '93010')
...
/*
medicare_count = FOREACH (GROUP filtered_medicare BY (nppes_provider_street1, nppes_provider_street2, nppes_provider_city, nppes_provider_state))
				 GENERATE COUNT(filtered_medicare.nppes_provider_street1);
					
STORE medicare_count INTO 'medicareCount';
*/
...
/*physician_count = FOREACH(GROUP grouped_physician BY (recipient_primary_business_street_address_line1, recipient_primary_business_street_address_line2, recipient_city, recipient_state))
					GENERATE COUNT(grouped_physician.recipient_primary_business_street_address_line1);					
STORE physician_count INTO 'physicianCount';					
*/				
...
/*join_count = FOREACH(GROUP joined_filtered_records ALL)
					GENERATE COUNT(joined_filtered_records.nppes_provider_street1);
					
STORE join_count INTO 'joinCount';
*/
